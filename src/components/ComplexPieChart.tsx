...
interface ComplexPieChartProps {
    dataSet: object[];
    path: [];
...
function ComplexPieChart(props: ComplexPieChartProps) {
	...
	useEffect(() => {
		setSelectedData(dataSet.filter(
			(species: object) => species[path] === selectedOption
		));
	}, [selectedOption, dataSet, path]);
	...
	const renderDropdown = () => (
		<div className="pie-chart-selection-options">
			<FormControl fullWidth>
				...
				>
					{selectionOptions.map((option: string | undefined, index: number) => (
...
export default ComplexPieChart;
